...
def export_to_excel(data: List[Dict[str, str]], output_file: str = "vehicle_data.xlsx") -> bool:
    ...
    Args:
        data: List of dictionaries with Brand, Model, Version, Engines (comma-separated)
    ...
    try:
        ...
        # Ensure column order
        column_order = ["Brand", "Model", "Version", "Engines"]
        df = df[column_order]
...
def get_dataframe(data: List[Dict[str, str]]) -> pd.DataFrame:
    ...
    """
    if not data:
        return pd.DataFrame(columns=["Brand", "Model", "Version", "Engines"])
